...
export async function generateActionPlanForCycle(
  cycleId: string,
  companyId: string
): Promise<{ created: number; skipped: boolean }> {
  // Check if plans already exist — do NOT delete, just skip
  const existing = await db.query.actionPlans.findFirst({
    where: and(
      eq(actionPlans.companyId, companyId),
      eq(actionPlans.cycleId, cycleId),
    ),
    columns: { id: true },
  })
  ...
  for (const score of scores) {
    ...
    const criticalResponses = await db.query.diagnosticResponses.findMany({
      where: and(
        ...
        eq(diagnosticResponses.dimensionId, score.dimensionId),
      ),
      with: { indicator: true },
